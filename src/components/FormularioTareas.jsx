...
const FormularioTareas = () => {
  ...
  const agregarTarea = async (dato) => {
    ...
    if (!dato.tarea || dato.tarea.trim() === "") {
      Swal.fire({
        title: "La tarea no puede estar vacía",
        icon: "warning",
        draggable: true,
      });
      return; // corta la función, no manda nada al backend
    }
    ...
    if (respuesta.status === 201) {
      ...
      Swal.fire({
        title: "No pudimos crear tu tarea",
        icon: "error",
        draggable: true,
      });
    }
    ...
    <div>
      ...
      <Form onSubmit={handleSubmit(agregarTarea)}>
        ...
        >
          ...
          />
          <Button variant="info ms-3" type="submit">
            ➕
          </Button>
        </Form.Group>
